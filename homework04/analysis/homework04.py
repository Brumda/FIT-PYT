...
def load_dataset(train_file_path: str, test_file_path: str) -> pd.DataFrame:
    ...
    df.drop(columns=["Ticket", "Embarked", "Cabin"], inplace=True)
...
def get_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    new_df["Total"] = [df[col].isnull().sum() for col in df.columns]
    new_df["Percent"] = [df[col].isnull().sum() / df.shape[0] * 100 for col in df.columns]
...
def substitute_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    new_df.Fare = new_df.Fare.fillna(15)
...
def get_survived_per_class(df: pd.DataFrame,
    ...
    # Implement your own solution
    grouped = df.groupby(group_by_column_name)["Survived"]
    percentage = (grouped.sum() / grouped.count()).round(2)
    return pd.Series(percentage, name="Survived").sort_values(ascending=False)
...
def determine_survival(df: pd.DataFrame, n_interval: int, age: float,
    ...
    new_df = new_df.sort_values(by="Age", ascending=True)
    new_df["Interval"] = pd.cut(new_df["Age"], bins=n_interval)

    filtered_df = new_df[new_df["Interval"].apply(lambda x: age in x)]
    if filtered_df[filtered_df["Sex"] == sex].empty:
        return np.NAN

    grouped = filtered_df.groupby("Sex")["Survived"]
    percentage = grouped.sum() / grouped.count()
    return percentage[sex]
